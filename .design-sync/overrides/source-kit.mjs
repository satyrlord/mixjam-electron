// forked from design-sync lib/source-kit.mjs — synth-entry `export *` never
// re-exports `export default` (standard ESM behavior), so a source tree of
// all-default-export components (this repo's convention) bundled to an empty
// window.<GLOBAL> and failed [BUNDLE_EXPORT]. This fork writes named
// re-exports instead, reusing the same ts-morph declaration scan
// deriveComponentsFromSrc already does to recover each default export's real
// name.
//
...
// Per-file named re-export statements for the synth entry — recovers each
// export's real name (unwrapping `export default function X()` to `X`) so
// default-exported components actually reach window.<GLOBAL>, unlike a blind
// `export * from`.
function namedReexportsFor(srcFiles) {
  const project = new Project({
    skipAddingFilesFromTsConfig: true,
    compilerOptions: { jsx: ts.JsxEmit.Preserve, allowJs: true, skipLibCheck: true },
  });
  ...
  for (const p of srcFiles) {
    ...
    for (const [name, decls] of sf.getExportedDeclarations()) {
      if (name === 'default') {
        ...
        if (real) specifiers.push(`default as ${real}`);
      } else {
        specifiers.push(name);
      }
    }
    if (specifiers.length) lines.push(`export { ${specifiers.join(', ')} } from ${JSON.stringify(p)};`);
  }
  return lines;
}
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    writeFileSync(entry, namedReexportsFor(comps).join('\n') + '\n');
    synthEntry = true;
    console.error(
