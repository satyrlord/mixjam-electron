...
web-first architecture port (writable Sample Folder handle — see
`tmp/mixjam-electron-handoff-web-first-thin-electron-shell.md`, Phase 4)
...
- The feature requires a writable Sample Folder handle; it never writes
  anywhere else.
...
- Format handling: many archive.org items are FLAC/OGG/78rpm MP3 — download
  ...
  in `src/main/path-utils.ts` defines what the indexer accepts today.)
...
- Web-first architecture handoff — `tmp/mixjam-electron-handoff-web-first-thin-electron-shell.md`
