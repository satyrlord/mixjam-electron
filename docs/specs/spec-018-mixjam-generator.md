...
**Depends on:** spec-003 (Folder & App State Management), spec-004 (Sample
Library), spec-008 (Sample Analysis), spec-011 (Project Save & Load, including
its generator metadata contract), spec-021 (Generator Arrangement Model)
...
profiles are bundled as auto-discovered JSON templates. Adding another valid
...
- **Empty or unavailable:** the card explains that the folder has no usable audio
  or cannot be read and offers the existing retry path.
...
While the wizard is open, focus stays inside it and ordinary Player and transport
hotkeys do not run. Closing it returns focus to the Home generator action or the
...
The result is at least eight bars, so every arrangement is phrased in eights — a
...
reads and before returning a plan. Cancellation before commit creates no file. If
an automatic sync request targets a root whose completed automatic job is already
...
Allocation is serialized inside the single-tab app and checks existing names. The
...
filesystem writers are outside this contract. The recent-project registry is
...
The template JSON field `version` is serialized as `profileVersion`.
...
key, sample type, top-level source-group name, and palette slot. Scan completion
timestamps are excluded because a no-op re-scan must preserve the fingerprint.
...
is not exact and must never be substituted silently. Current-corpus regeneration
...
whose exact profile ID/version pair is registered by the running app.
...
- [x] **AC-003:** The Parameters step exposes every validated bundled template in
  deterministic registry order, selected-cluster BPM, medium intensity, editable
  30–600 second duration, and a validated safe-token seed. A mixed root requires
  coherent cluster selection and never defaults from a root-wide median.
- [x] **AC-004:** Duration uses nearest whole-phrase, half-up rounding and the
  generated project ends exactly at the resulting bar boundary with no
...
- [x] **AC-007:** With the same seed, registered profile ID and profile version,
  ...
  semantically equivalent plans, including the same seeded section arc.
- [x] **AC-008:** A generated project roundtrips in full through the strict
  spec-011 parser — not only its generator block — so every placement, lane, and
  bus it emits satisfies the loader that will read it back. This includes
  spec-011 AC-016: all placements of one `sampleRef` carry the same
  `durationTicks`, so a scheduler that shortens a sample at one site shortens it
  everywhere. Placement and lane IDs derive only from stable seeded inputs —
  never from wall-clock time, a UUID, or array position.
- [x] **AC-009:** Planner scoring consumes analyzer-owned BPM, key, and sample
...
- `src/shared/backend-api.ts` exposes profile IDs as validated registry strings,
  not a closed union.
- The spec-008 analyzer owns group readiness, raw BPM/key evidence, the stored
  BPM/key/type projections used by generator queries, and the pool token.
...
- `project/generated-project.ts`, `hooks/useMixJamGenerator.ts`, and
  ...
  materialize the declared return buses, expose cancellation and progress, render
  profile choices from registry metadata, and keep Open in Player explicit.
...
read/analysis counts, proof that stored BPM/key/type remain unchanged, palette
slots, placement count, final tick, missing references, parser roundtrip,
playback proof, screenshots, and listening notes. Include a mixed-root case in
...
capacity boundary. Negative fixtures cover malformed JSON-shaped values, unknown
schema fields and versions, filename/ID mismatch, duplicate IDs, duplicate lane,
section, or arc names, multiple defaults, invalid lane references, unsupported
acoustic types, out-of-range numeric values, over-wide template pan, mismatched
send vectors, and ops naming unknown sections or unsuitable lanes. A
planning-boundary test asserts that rejection occurs before any corpus, query,
