...
project is loaded) and a **Tracker View** (displayed when a project is active).
...
- **US-002:** As a user, I click "Start New MixJam" and see the full tracker layout appear so I can start arranging.
- **US-003:** As a user, I see the app brand and a timer in the header while working in the tracker so I know what app I'm using and how long I've been working.
- **US-004:** As a user, I can return to the Home Screen from the tracker via a home link so I can start over.
...
- **Content area:** vertically and horizontally centered.
  ...
  - "Load MixJam" link — secondary action, opens a native file picker filtered
    ...
    Tracker View. If the user cancels the picker, stays on the Home Screen.
...
- Header (40px): home link "&lt; Return to Main Menu" (left, only in
  Tracker View), brand "MixJam Electron" (right of home link), timer
...
- Clicking "Start New MixJam" on the Home Screen replaces the content area
  with the Tracker View.
- Clicking "Load MixJam" opens the native file picker. If the user selects a
  file, the content area switches to the Tracker View (the file is not
...
- Clicking the home link "&lt; Return to Main Menu" in the tracker header
  returns to the Home Screen.
- View switching must be instantaneous (no page reload, no navigation delay).
- When switching from Home to Tracker, the window resizes from 1280×720 to
  1920×1080 and the maximize button becomes available.
- When switching from Tracker to Home, the window resizes from its current
...
- [ ] **AC-004:** Clicking "Start New MixJam" resizes the window to 1920×1080 centered, enables the maximize button, and switches the content area to the Tracker View.
- [ ] **AC-005:** In Tracker View, the header shows home link "&lt; Return to Main Menu", brand "MixJam Electron", and timer (`00:00.0`).
- [ ] **AC-005a:** The home link "&lt; Return to Main Menu" is NOT present in the Home Screen header. It only appears in the Tracker View header.
- [ ] **AC-006:** The timer is absolutely centered in the header — it does not shift when left/right content changes.
- [ ] **AC-007:** Clicking "Load MixJam" opens a native file picker. Selecting a file navigates to Tracker View (with window resize). Cancelling the picker stays on the Home Screen.
- [ ] **AC-008:** Clicking the home link "&lt; Return to Main Menu" in the tracker header resizes the window back to 1280×720, removes the maximize button, and returns to the Home Screen.
- [ ] **AC-009:** Roundtrip: Home → Tracker → Home → Tracker works without visual glitches or state leaks, and window dimensions are correct at each step.
- [ ] **AC-010:** The tracker content area shows three empty labeled rectangular zones (timeline, transport, browser) — no lane rows, no icons, no sub-zones.
...
- No theme switching — the app renders with a single hardcoded default look.
  Theming is spec-002.
- No real audio playback, no transport controls, no BPM — all tracker content
