...
2. Read one file and its sibling completely, then load the applicable branch
   reference.
3. Mark only differences that lack a behavioral, informational, or local-style
   reason.
4. Remove the smallest proven slop. Preserve behavior, contracts, historical
   constraints that remain active, and intentional voice.
5. Run the narrowest relevant validation after each coherent edit group.
6. Re-read every scoped file after the pass and account for it in the final
   report.
