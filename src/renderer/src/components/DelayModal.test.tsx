...
describe('DelayModal', () => {
  it('uses accessible sliders, blocks the app, previews, saves, and restores focus', () => {
    ...
    const freeTime = screen.getByRole('slider', { name: 'Free time' })
    expect(freeTime).toHaveAttribute('aria-valuetext', '375 ms')
    expect(freeTime).toHaveClass('linear-slider-thumb')
    expect(freeTime.closest('.linear-slider')).toHaveAttribute('data-orientation', 'horizontal')
    expect(freeTime.querySelector('.linear-slider-handle')).toBeInTheDocument()
    fireEvent.keyDown(freeTime, { key: 'ArrowUp' })
